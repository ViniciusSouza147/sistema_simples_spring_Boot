...
import br.com.springbootgreendogdelivery.repository.ItemRepository;
...
public class ItemController {
	...
	public ModelAndView list() {
		Iterable<Item> item = this.itemRepository.findAll();
		return new ModelAndView(ITEM_URI + "list","item", item);
	}
	...
	public ModelAndView create(@Valid Item item, BindingResult result, RedirectAttributes redirect) {
		...
		redirect.addFlashAttribute("globalMassage", "Item gravado com sucesso");
		return new ModelAndView("redirect:/" + ITEM_URI, "{item.id}", item.getId());		
	} 
	...
	public ModelAndView remove(@PathVariable("id") Long id, RedirectAttributes redirect) {
		...
		mv.addObject("glocalMessage", "Item removido com sucesso.");
